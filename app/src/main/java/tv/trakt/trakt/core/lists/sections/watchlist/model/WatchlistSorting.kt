...
internal fun getWatchlistSorting(sort: Sorting?): Comparator<WatchlistItem> {
    if (sort == null) {
    ...
    return when (sort.type) {
        ...
        Added -> when (sort.order) {
            Asc -> compareBy<WatchlistItem> { it.listedAt }
            Desc -> compareByDescending { it.listedAt }
        }.thenBy { it.rank }

        Runtime -> when (sort.order) {
            Asc -> compareBy { it.totalRuntime }
            Desc -> compareByDescending { it.totalRuntime }
        }
