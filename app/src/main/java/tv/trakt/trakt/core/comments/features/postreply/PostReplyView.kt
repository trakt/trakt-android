...
@Composable
internal fun PostReplyView(
    viewModel: PostReplyViewModel,
    ...
    ViewContent(
        state = state,
        gifQuery = gifQuery,
        onSubmitClick = { reply, spoiler, gif ->
...
private fun ViewContent(
    ...
    // A picked GIF is content on its own, so the minimum word count applies to text-only replies.
