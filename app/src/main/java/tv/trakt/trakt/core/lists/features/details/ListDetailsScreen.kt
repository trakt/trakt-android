...
private fun ContentList(
    ...
    LazyColumn(
        ...
        if (listItems.isNotEmpty()) {
            itemsGroup.keys.forEachIndexed { groupIndex, key ->
                key?.let {
                    item(
                        key = "header-$key",
                    ) {
                        TraktHeader(
                            ...
                            modifier = Modifier
                                .padding(
                                    top = if (groupIndex == 0) 0.dp else 16.dp,
                                    bottom = 12.dp,
                                )
                                .animateItem(
                                    fadeInSpec = null,
                ...
                items(
                    ...
                    when (item) {
                        is ShowItem -> ListDetailsShowView(
                            ...
                            plays = collectionState.plays(item.id, item.type, item.show.airedEpisodes),
                        ...
                        is MovieItem -> ListDetailsMovieView(
                            ...
                            plays = collectionState.plays(item.id, item.type, null),
                            watchlist = collectionState.isWatchlist(item.id, item.type),
                            showIcon = true,
                            onClick = { onClick(item) },
                            onLongClick = { onLongClick(item) },
