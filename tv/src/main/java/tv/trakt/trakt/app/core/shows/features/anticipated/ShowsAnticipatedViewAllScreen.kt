...
internal fun ShowsAnticipatedScreen(
    viewModel: ShowsAnticipatedViewAllViewModel,
...
@Composable
private fun ShowsAnticipatedContent(
    state: ShowsAnticipatedViewAllState,
        ...
        LazyVerticalGrid(
            ...
            if (state.isLoading && state.shows.isNullOrEmpty()) {
                ...
                items(
                    ...
                    val anticipatedShow = state.shows[index]
                    val focusRequester = focusRequesters.getOrPut(anticipatedShow.show.ids.trakt.value) {
                        FocusRequester()
                    }

                    VerticalMediaCard(
                        title = anticipatedShow.show.title,
                        imageUrl = anticipatedShow.show.images?.getPosterUrl(),
                        onClick = {
                            if (!state.isLoadingPage) {
                                onShowClick(anticipatedShow.show.ids.trakt)
                            }
                        },
                        chipContent = {
                            InfoChip(
                                text = rememberThousandsFormat(anticipatedShow.listCount),
                        ...
                        modifier = Modifier
                            ...
                            .onFocusChanged {
                                if (it.isFocused) {
                                    focusedShow = anticipatedShow.show
                                    focusedShowId = anticipatedShow.show.ids.trakt.value
...
@Preview
@Composable
private fun ShowsAnticipatedContentPreview() {
    TraktTheme {
        ShowsAnticipatedContent(
            state = ShowsAnticipatedViewAllState(
                shows = listOf(
